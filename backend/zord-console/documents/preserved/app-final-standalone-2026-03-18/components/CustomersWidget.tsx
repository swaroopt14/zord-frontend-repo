  ...
  return (
    <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
      ...
      <div className="flex items-end justify-between gap-4">
        <div className="text-5xl font-bold">1,284</div>
        ...
        <div className="flex flex-wrap gap-2 justify-end w-40">
          {[...Array(35)].map((_, i) => {
            ...
            const baseIntensity = Math.sin(i * 0.3) * 2
            const peakOffset = Math.abs(i - 12)
            const intensity = Math.max(0, 3.5 - peakOffset * 0.25 + baseIntensity)
            const colorIndex = Math.floor(Math.max(0, Math.min(4, intensity)))
            
            return (
              <div
                key={i}
                className={`w-2.5 h-2.5 rounded-full ${colors[colorIndex]}`}
              ></div>
            )
          })}
        </div>
      ...
      <div className="mt-8 text-right">
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <span className="text-lg font-semibold text-gray-900">+320</span>
        </div>
      </div>
    </div>
  )
